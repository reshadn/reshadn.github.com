...
Mint.save = function() 
{
	...
	var path	= 'http://awesomejs.com/mint/?record&key=37416d717a493558684436313636363533316d';
...
{
	return /^([^:]+):\/\/([a-z0-9]+[\._-])*(awesomejs\.com)/i.test(url);
};
function Mint_SI_IO_save()
{
	var now		= new Date();
	var then	= now.getTime() + 300;
	var path	= 'http://awesomejs.com/mint/pepper/shauninman/outbound/click.php?'+now.getTime();
	path 		= path.replace(/^https?:/, window.location.protocol);
